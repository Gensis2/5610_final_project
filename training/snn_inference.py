...
def run(batch_size=64, num_steps=10, lr=0.1, leak_mem=0.99, num_epochs=50, img_size=28, num_classes=10, case_study='accuracy', df=None):
    ...
    if case_study == 'accuracy':
        new_df = pd.DataFrame([[batch_size, lr, leak_mem, num_steps, num_epochs, train_loss.avg, format(test_accuracy)]], columns=['batch_size', 'lr', 'leak_mem', 'num_steps', 'num_epochs', 'training_loss', 'testing_accuracy'])
    elif case_study == 'flops':
        df = pd.DataFrame([[batch_size, lr, leak_mem, num_steps, num_epochs, model.total_flops]], columns=['batch_size', 'lr', 'leak_mem', 'num_steps', 'num_epochs', 'flops'])
    else:
        new_df = pd.DataFrame([[batch_size, lr, leak_mem, num_steps, num_epochs, train_loss.avg, format(test_accuracy), model.total_flops]], columns=['batch_size', 'lr', 'leak_mem', 'num_steps', 'num_epochs', 'training_loss', 'testing_accuracy', 'flops'])
