...
def run(batch_size=64, lr=0.1, num_epochs=50, epoch_list=None, img_size=28, num_classes=10, case_study='accuracy', df=None):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch+1 in epoch_list:
            ...
            if case_study == 'accuracy':
                new_df = pd.DataFrame([[batch_size, lr, epoch+1, train_loss.avg, format(test_accuracy)]], columns=['batch_size', 'lr', 'num_epochs', 'training_loss', 'testing_accuracy'])
            else:
                new_df = pd.DataFrame([[batch_size, lr, epoch+1, train_loss.avg, format(test_accuracy), model.total_flops]], columns=['batch_size', 'lr', 'num_epochs', 'training_loss', 'testing_accuracy', 'flops'])
            df = pd.concat([df, new_df], ignore_index=True)
    
    if case_study == 'flops':
        df = pd.DataFrame([[batch_size, lr, epoch+1, model.total_flops]], columns=['batch_size', 'lr', 'num_epochs', 'flops'])
